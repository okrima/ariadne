...
template<class X> requires Constructible<X,ExactDouble> Vector<X> make_vector(InitializerList<double> lst) {
    return Vector<X>(Array<ExactDouble>(Array<double>(lst))); }

template<class X> requires Constructible<X,ExactDouble,DoublePrecision> Vector<X> make_vector(InitializerList<double> lst) {
