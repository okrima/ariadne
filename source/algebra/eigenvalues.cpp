...
// FIXME: Should be in FloatBounds class
Value<FloatMP> midpoint(Bounds<Ariadne::FloatMP> const& x) {
    return x.value();
}
...
template<class X> Pair<X,Vector<X>> eigenvalue_solve(Matrix<X> const& A, X mu, Vector<X> v, ValidatedTag) {
    ...
    while(step<3) {
        ...
        Matrix<X> Dr(n+1,n+1,z);
        project(Dr,range(0,n),range(0,n))=A-mu*Matrix<X>::identity(n);
