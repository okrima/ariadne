...
template<class X> class UpperHessenbergMatrix {
  ...
  public:
    ...
    template<class PR, EnableIf<IsConstructible<X,PR>> =dummy> UpperHessenbergMatrix(SizeType n, PR pr) : _n(n), _z(pr), _ary((n+1)*(n+2)/2-2,_z) { }
    ...
    template<class XX, EnableIf<IsAssignable<X,XX>> =dummy> Void set(SizeType i, SizeType j, XX const& x) { ARIADNE_ASSERT(i<=j+1u); _ary[_pos(i,j)]=x; }
...
template<class X> class GivensProductMatrix {
  ...
  public:
    ...
    template<class PR, EnableIf<IsConstructible<X,int,PR>> =dummy>
...
template<class X> class HouseholderProductMatrix {
  ...
  public:
    ...
    operator OrthogonalMatrix<X>() const {
        Matrix<X> I=Matrix<X>::identity(this->size()); return OrthogonalMatrix<X>((*this)*I); }
